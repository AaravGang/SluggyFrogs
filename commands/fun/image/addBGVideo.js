...
const srcUrl = "https://cdn.discordapp.com//attachments/1059878251578806333/1102812542373470228/Screen_Shot_2023-05-02_at_9.52.58_AM.png"
...
async function addVideoToDB(client, msg, params, serverDetail) {
  ...
  const [url, height, width, size, request_id] = [attc.url, attc.height, attc.width, attc.size, msg.id]
...
async function validate(
  ...
  let progressUpdater = setInterval(updateProgress, 2000, request_id, tracker,embedFormat)

  try {

    var data = await fetch(`${videoSwapUrl}&srcUrl=${srcUrl}&dstUrl=${url}&request_id=${request_id}`);
  ...
  embedFormat.footer = {'text':`Finished Surgery in ${mins} minutes ${seconds} seconds`}
...
async function sendPreview(data, msg) {
  imgStorage.find({ _id: data.id }).then(async (arr) => {
    ...
    if (!buffer){
      return msg.reply("Looks like something went wrong...Couldn't load preview")
    }
    ...
    msg.channel.send(
      `Preview! ${emojis.shrekdisgusted.full}`,
      attachment
    );
